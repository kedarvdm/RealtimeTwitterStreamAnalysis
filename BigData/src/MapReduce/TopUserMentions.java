...
public class TopUserMentions {
        ...
        public void map(ImmutableBytesWritable rowKey, Result columns, Mapper.Context context) throws IOException, InterruptedException {
            try {
                ...
                for (String text : Hashtags) {
                    if (!(text.equals("@") || text.equals("@null")) && text.trim().startsWith("@")) //Checking if it a Hashtag
                    {
                        kvt.setKey(text);
                        kvt.setValue(0);
    ...
    public static class ReducerClass extends Reducer<Text, LongWritable, Text, LongWritable> {
        ...
        private SortedSet<KeyValueTuple> userMentions = new TreeSet<>();
        ...
        public void reduce(Text key, Iterable<LongWritable> values, Context context) throws IOException, InterruptedException {
            ...
            kvt.setValue(total);
            userMentions.add(kvt);
        }
        
        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
            int i=0;
            String filename = TweetUtils.OUTPUT_PREFIX+"TopUserMentions/TopUserMentions.csv";
            ArrayList<KeyValueTuple> topUserMentions= new ArrayList<>();
            for (KeyValueTuple kvt : userMentions) {
                if(i==10) break;
                topUserMentions.add(kvt);
                i++;
            }
            CSVUtils.CSVUtils.createTopUserMentionsCSV(topUserMentions, filename);
        }
    }
