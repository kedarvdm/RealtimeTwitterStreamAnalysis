 ...
 * @author kedarvdm
 */
public class TweetCountPerState {
    ...
    public static class MapClass extends TableMapper<Text, LongWritable> {
        ...
        public void map(ImmutableBytesWritable rowKey, Result columns, Mapper.Context context) throws IOException, InterruptedException {
            try {
                ...
                String placeCountryCode= Bytes.toStringBinary(columns.getValue(Bytes.toBytes("place"), Bytes.toBytes("placecountrycode")));
                if(placeCountryCode.equals("US"))
                {
                    String placeFullName= Bytes.toStringBinary(columns.getValue(Bytes.toBytes("place"), Bytes.toBytes("placefullname")));
                
                //if(!placeFullName.equals("null"))
                //{
                    placeFullName= new String(columns.getValue(Bytes.toBytes("place"), Bytes.toBytes("placefullname")),"UTF-8");
                    String state= TweetUtils.findState(placeFullName);
                    if(!state.equals(""))
                    {
                        kvt.setKey(state);
                        kvt.setValue(0);
    ...
    public static class ReducerClass extends Reducer<Text, LongWritable, Text, LongWritable> {
        ...
        private SortedSet<KeyValueTuple> tweetsPerState = new TreeSet<>();
        ...
        public void reduce(Text key, Iterable<LongWritable> values, Context context) throws IOException, InterruptedException {
            ...
            kvt.setValue(total);
            tweetsPerState.add(kvt);
        }

        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
            int i = 0;
            String filename = TweetUtils.OUTPUT_PREFIX+"TweetsPerState/TweetsPerState.js";
            ArrayList<KeyValueTuple> tweetsPerStateCount= new ArrayList<>();
            for (KeyValueTuple kvt : tweetsPerState) {
                tweetsPerStateCount.add(kvt);
            }
            CSVUtils.CSVUtils.generateGeoJs(tweetsPerStateCount, filename);
        }
    }
